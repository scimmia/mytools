...
def export_mid_file(dates, teacher_classes):
    ...
    wsa = work_book.add_worksheet('排课A')
    ...
    wsb = work_book.add_worksheet('排课')
    ...
    write_teacher_info_sheet(work_book, teacher_classes)
...
def export_course(teacher_dict, dates, class_dict, teacher_datas):
    work_book = xlsxwriter.Workbook(
        path_course_file.get() + '--排课表' + datetime.datetime.now().strftime('%H%M') + '.xlsx')
    head_format = work_book.add_format(format_header)
    merge_format = work_book.add_format(format_merge)
    normal_format = work_book.add_format(format_normal)

    write_date_class_sheet(work_book, dates, class_dict)
    write_teacher_info_sheet(work_book, teacher_dict)
    for class_name, teachers in class_dict.items():
        ...
        ws.write_row(2, 2, ['课程设置', '师资方', '单位', '讲师', '备注'], merge_format)
        start_row = 3
        write_dates_to_sheet(ws, dates, merge_format, 3)
        ...
        #     size = len(values)
        #     if size > 1:
        #         ws.merge_range(start_row, 0, start_row + size - 1, 0, keys, head_format)
        #     else:
        #         ws.write(start_row, 0, keys, head_format)
        #     for value in values:
        #         ws.write(start_row, 1, value, merge_format)
        #         teacher = teacher_dict[teachers[start_row - 3]]
        #         ws.write_row(start_row, 2, teacher.get_class_data(), normal_format)
        #         start_row += 1
    ...
    for name, datas in teacher_datas.items():
        ...
        for date, value in row_datas.items():
            ws.write(startrow + index, 0, date, merge_format)
            ws.write_row(startrow + index, 1, value.values(), normal_format)
            index += 1
...
# 生成教师信息表
def write_teacher_info_sheet(work_book, teacher_classes):
    ...
    for value in teacher_classes.values():
        ws.write_row(start_row, 0, value.get_data())
        start_row += 1
...
def make_course():
    ...
    if True:
        ...
        teacher_course = get_teacher(book)
        for teacher_name in teacher_course.keys():
        ...
        for rx in range(1, sh.nrows):
            ...
            if row[col_date].ctype == 1:
                ...
                the_date = temp.strftime('%m-%d')
            for ix in range(2, sh.ncols):
                teacher_name = row[ix].value
                teacher_datas[teacher_name].append((the_date, row[1].value, row_one[ix].value))

        export_course(teacher_course, dates, class_courses, teacher_datas)
        pass
...
def main():
    ...
    Button(root, text='生成排班', command=make_mid_file).grid(row=4, column=1, )
...
chVarDis = BooleanVar()
chVarDis.set(False)
check1 = Checkbutton(root, text="从排课表生成", variable=chVarDis)
