...
from utils import get_city_org
import xlrd
import xlsxwriter
...
def startIt():
    ...
    try:
        ...
        for rx in range(row_index, sh.nrows):
            ...
            result = get_city_org(row[org_index])
            if result is not None:
                # print(row)
                add_to_results(results, result[1], row)
                pass
        showlog('已完成')
...
def main():
    ...
    Button(root, text='开始', command=startIt).grid(row=5, column=1, columnspan=1)
    root.mainloop()
