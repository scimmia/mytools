...
def export_file(all_datas, class_rows):
    work_book = xlsxwriter.Workbook(pathFile.get() + '分班' + datetime.datetime.now().strftime('%H%M') + '.xlsx')
    ...
    the_class = classes.get().split(" ")
    for c in the_class:
        worksheets[c] = init_sheet(work_book, c)
        ...
        for k, v in values.items():
            ...
            if n == 0:
                ...
                if size > 2:
                    ...
                    sheet_city.merge_range(a, key, normal_format)
                    sheet_city.merge_range(b, '=SUM(D%d:D%d)' % (m + 1, m + size), normal_format)
                else:
                    sheet_city.write(m, 0, key, normal_format)
                    sheet_city.write(m, 1, '=SUM(D%d:D%d)' % (m + 1, m + size), normal_format)
            sheet_city.write(m, 2, k, normal_format)
            sheet_city.write(m, 3, len(v), normal_format)
            m = m + 1
            n = n + 1

            for temp in v:
                showlog(temp)
                class_name = the_class[(startrow - 2) % len(the_class)]
                ws_all.write(startrow, 0, str(startrow - 1), normal_format)
                ws_all.write(startrow, 1, key, normal_format)
                ws_all.write_row(startrow, 2, temp, normal_format)
                ws_all.write(startrow, len(header_titles) + 1, class_name, normal_format)
                ws_all.write(startrow, len(header_titles), '', normal_format)

                row_temp = int(1 + (startrow - 2) / 5)
                ws_temp = worksheets[class_name]
                ws_temp.write(1 + row_temp, 0, str(row_temp), normal_format)
                ws_temp.write(1 + row_temp, 1, key, normal_format)
                ws_temp.write_row(1 + row_temp, 2, temp, normal_format)
                ws_temp.write(1 + row_temp, len(header_titles), '', normal_format)

                startrow = startrow + 1
    sheet_city.write(m, 0, '合计', normal_format)
    sheet_city.write(m, 1, '=SUM(D:D)', normal_format)

    for wrong_result in wrong_results:
        ws_all.write_row(startrow, 2, wrong_result, normal_format)
        startrow = startrow + 1
...
def make_file():
    work_book = xlsxwriter.Workbook(pathFile.get() + '分班.xlsx')
