...
def add_to_results(all_result,the_results,org,row):
...
def wirte_results(sheet_summary, normal_format,the_result):
    ...
    for city,org_temp in the_result.items():
        ...
        for org,row in org_temp.items():
            sheet_summary.write(m, 0, city, normal_format)
            sheet_summary.write(m, 1, org, normal_format)
            sheet_summary.write(m, 2, row, normal_format)
            m = m + 1
...
def startIt():
    ...
    for rx in range(start_row.get()-1, sh.nrows):
        ...
        res = add_to_results(all_result,the_result,org,row)
        if res == -1:
            showlog(row[col])
            error_count += 1
