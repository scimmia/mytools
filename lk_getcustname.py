...
def doIt():
    ...
    with open(pathFile.get(), 'r') as f:
        for line in f:
            custs = line.split('      ', 1)
            custno = custs[0]
            custname = custs[1].replace(" ",'').replace("\n",'')
            if not results.__contains__(custno):
                results[custno] = custname
    with open(pathFilea.get(), 'r',encoding='UTF-8') as f:
        for line in f:
            custs = line.split('\t', 1)
            custorg = custs[0]
            custno = custs[1].replace(" ",'').replace("\n",'')
            name = 'error'
            if results.__contains__(custno):
                name = results[custno]
            else:
                error_count += 1

            showlog(custorg+'\t'+custno + '\t' + name)
    print(error_count)
    showlog('finished')
...
def main():
    ...
    Button(root, text='选择成绩文件', command=selectFilea).grid(row=1, column=1)
    Entry(root, textvariable=pathFilea).grid(row=1, column=2)
